...
def test_single_file_upload_json(content, running_server):
    ...
    assert req.json() == {
        "success": True,
        "redirect": mock.ANY,
        "uploaded_files": {"ohai.bin": {"paste": mock.ANY, "raw": mock.ANY}},
    }
...
def test_multiple_files_upload(running_server):
    ...
    req = requests.post(running_server["home"] + "/upload", files=files)
    assert req.status_code == 200
...
def test_multiple_files_upload_json(running_server):
    ...
    req = requests.post(running_server["home"] + "/upload?json", files=files)
    assert req.status_code == 200
    assert req.json() == {
        "success": True,
        "redirect": mock.ANY,
...
def test_plaintext_files_are_also_pasted(content, running_server):
    ...
    assert (
        pq(req.content.decode("utf8")).find("input[name=text]").attr("value") == content
    )
