...
def test_simple_paste_from_file(content, running_server, tmpdir):
    ...
    assert_url_matches_content(
        raw_text_url_from_paste_html(req.text), content.encode("UTF-8")
    )
...
def test_simple_paste_from_stdin(content, running_server, tmpdir):
    info_url = subprocess.check_output(
        ("fpb", "--server", running_server["home"]), input=content.encode("UTF-8")
    ).strip()
...
def test_paste_with_tee(running_server, tmpdir):
    ...
    output = subprocess.check_output(
        ("fpb", "--server", running_server["home"], "--tee"), input=input_text
    ).strip()
    ...
    assert_url_matches_content(raw_text_url_from_paste_html(req.text), input_text)
