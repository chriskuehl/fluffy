...
FILE_CONTENT_TESTCASES = (
    tuple(content.encode("utf8") for content in PLAINTEXT_TESTCASES) + BINARY_TESTCASES
)


def urls_from_details(details):
    """Return list of URLs to objects from details page source."""
    return re.findall(r'<a href="(http://localhost:\d+/object/[^"]+)"', details)


def paste_urls_from_details(details):
    """Return list of URLs to objects from details page source."""
    return re.findall(r'<a href="(http://localhost:\d+/html/[^"]+)"', details)
