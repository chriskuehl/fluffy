        ...
        try:
            with open(path) as f:
                ...
                if not isinstance(j, dict):
                    raise ValueError(
                        'Expected to parse dict, but the JSON was type "{}" instead.'.format(
                            type(j)
                        )
                    )
            ...
            print(
                bold('Error parsing config file "{}". Is it valid JSON?'.format(path))
            )
            raise
    return config
...
def upload(server, paths, auth, direct_link):
    files = (
        ("file", sys.stdin.buffer if path == "-" else open(path, "rb"))
        for path in paths
    )
    req = requests.post(
        server + "/upload?json", files=files, allow_redirects=False, auth=auth
    )
    ...
    if req.status_code != 200:
        ...
        if highlight_regex:
            ...
            if matches:
                location += "#" + ",".join(
                    "L{}".format(
                        "{}-{}".format(*match) if match[0] != match[1] else match[0]
                    )
...
def upload_main(argv=None):
    ...
    parser.add_argument(
        "--server", default=config["server"], type=str, help="server to upload to"
    )
    parser.add_argument(
        "--version", action="version", version="%(prog)s {}".format(__version__)
    )
    parser.add_argument(
        "--auth", dest="auth", action="store_true", help="use HTTP Basic auth"
    )
    parser.add_argument(
        "--no-auth",
        dest="auth",
        action="store_false",
        help="do not use HTTP Basic auth",
    )
    ...
    parser.add_argument(
        "--direct-link", action="store_true", help="return direct links to the uploads"
    )
    parser.add_argument(
        "file", type=str, nargs="+", help="path to file(s) to upload", default="-"
    )
...
def paste_main(argv=None):
    ...
    parser.add_argument(
        "-r", "--regex", type=re.compile, help="regex of lines to highlight"
    )
    parser.add_argument(
        "--auth", dest="auth", action="store_true", help="use HTTP Basic auth"
    )
    ...
    parser.add_argument(
        "--direct-link",
        action="store_true",
        help="return a direct link to the text (not HTML)",
    )
    parser.add_argument(
        "--tee",
        action="store_true",
        help="Stream the stdin to stdout before creating the paste",
    )
    parser.add_argument(
        "file", type=str, nargs="?", help="path to file to paste", default="-"
    )
    args = parser.parse_args(argv)
    auth = None
    if args.auth:
